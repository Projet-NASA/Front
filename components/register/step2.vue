  ...
  <section class="bg-background-default min-h-screen flex justify-center items-center w-full">
    <div class="w-full sm:w-3/4 md:w-1/2 lg:w-1/3 xl:w-1/4 my-8">
      <form @submit.prevent="submitForm" class="bg-background-200 shadow-md rounded px-8 pt-6 pb-8 mb-2">
        ...
        <div class="mb-4">
          <label for="country" class="block text-text-default text-sm font-bold mb-2">Pays</label>
          <div class="relative">
            <input id="country" name="country" v-model="formData.country" type="text" @change="v$.country.$touch"
        ...
        'border-[#42d392] ': !v$.country.$invalid,
            ...
            <Icon v-if="!v$.country.$invalid || v$.country.$error"
        ...
        <div class="mb-3">
          <label for="city" class="block text-text-default text-sm font-bold mb-2">Ville</label>
          <div class="relative">
            <input id="city" name="city" v-model="formData.city" type="text" @change="v$.city.$touch"
            class="bg-background-default shadow appearance-none border-primary-800 border rounded w-full py-2 px-3 text-text-default mb-3 leading-tight focus:outline-none focus:shadow-outline"
              :class="{
        'border-red-500 focus:border-red-500 align-middle': v$.city.$error,
        'border-[#42d392] ': !v$.city.$invalid,
      }" />
            <Icon v-if="!v$.city.$invalid || v$.city.$error" class="absolute right-2 h-full text-xl text-green-500"
        ...
        <div class="mb-3">
          <label for="city" class="block text-text-default text-sm font-bold mb-2">Date de naissance</label>
...
<div class="relative">
  <input
        ...
        <div class="mb-6">
          ...
          <div class="relative">
            <input id="phone" name="phone" v-model="formData.phone" type="text" @change="v$.phone.$touch"
            class="bg-background-default shadow appearance-none border-primary-800 border rounded w-full py-2 px-3 text-text-default mb-3 leading-tight focus:outline-none focus:shadow-outline"
              :class="{
        'border-red-500 focus:border-red-500': v$.phone.$error,
        'border-[#42d392] ': !v$.phone.$invalid,
      }" />
            <Icon v-if="!v$.phone.$invalid || v$.phone.$error" class="absolute right-2 h-full text-xl text-green-500"
